...
class TinyPHP_Libs_Imap_Mailbox {
	...
	protected function connect() {
		...
		if(!$this->mbox) {
			throw new TinyPHP_Libs_Imap_MailboxException('Connection error: ' . imap_last_error());
		}
	}
	...
	public function setMailAsSeen($mId) {
        ...
        public function moveMail($mId, $destinationMailBox){
            $this->checkConnection();
            $rtn =  $this->imap_mail_move($this->mbox, $mId, $destinationMailBox,CP_UID);
            return $rtn;
        }
	...
	protected function getMailHeaders($mId) {
		...
		if(!$headers) {
			throw new TinyPHP_Libs_Imap_MailboxException('Message with UID "' . $mId . '" not found');
		}
		return $headers;
	}
	...
	public function getMail($mId) {
		...
		$mail = new TinyPHP_Libs_Imap_IncomingMail();
		$mail->mId = $mId;
	...
	protected function initMailPart(TinyPHP_Libs_Imap_IncomingMail $mail, $partStruct, $partNum) {
